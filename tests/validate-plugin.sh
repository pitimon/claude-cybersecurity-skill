...
# 3.1 All 15 reference files exist and are non-trivial (>100 lines)
EXPECTED_REFS=(
  ir-playbooks.md
  dfir-reports.md
...
# 3.3 Decision tree references all 15 files
  ...
  if [[ -d "$CACHE_DIR" ]]; then
    ...
    if [[ -n "$CACHE_VERSION_DIR" ]]; then
      ...
      if [[ -d "$CACHE_REFS_DIR" ]]; then
        ...
        if (( cache_ref_count >= 15 )); then
          pass "Cache has all $cache_ref_count reference files"
        else
          fail "Cache has only $cache_ref_count reference files (expected 15)"
        fi
      else
